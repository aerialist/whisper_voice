...
- 再合成: librosa.feature.inverse.mel_to_audio (Griffin-Lim)
...
def enhance_wav(in_wav, out_wav, model_path=MODEL_PATH):
    ...
    # Griffin-Limは振幅スペクトルを要求するので exp → power -> amplitude に変換
    Yhat_mel_power = np.exp(Yhat_mel_log).T  # [40, T]
    # librosa はパワー or 振幅Melを受けるが、mel_to_audio(power=2.0想定)でよい
    y_rec = librosa.feature.inverse.mel_to_audio(
        ...
        fmin=FMIN,
        fmax=FMAX,
        power=POWER,  # S=|X|**power で計算された前提を守る
...
def bulk_enhance(in_dir, out_dir, model_path=MODEL_PATH):
    ...
    for in_wav in tqdm(wav_files, desc="Bulk enhancing"):
        ...
        try:
            ...
            Yhat_mel_power = np.exp(Yhat_mel_log).T
...
def main():
    ...
    if args.enhance:
        enhance_wav(args.enhance[0], args.enhance[1], model_path=args.model_path)
    if args.bulk:
        bulk_enhance(args.bulk[0], args.bulk[1], model_path=args.model_path)
